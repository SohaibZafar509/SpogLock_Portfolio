...
(function ($) {
	...
	// Route homepage service cards and portfolio filters.
	...
	const portfolioFilter = new URLSearchParams(window.location.search).get('filter');
	if (portfolioFilter) {
		const portfolioFilterTabs = {
			all: '#nav-home-tab',
			web: '#nav-webapps-tab',
			webapp: '#nav-webapps-tab',
			webapps: '#nav-webapps-tab',
			'web-app': '#nav-webapps-tab',
			'web-apps': '#nav-webapps-tab',
			mobile: '#nav-mobile-tab',
			mobileapp: '#nav-mobile-tab',
			mobileapps: '#nav-mobile-tab',
			'mobile-app': '#nav-mobile-tab',
			'mobile-apps': '#nav-mobile-tab',
			product: '#nav-product-tab',
			'product-design': '#nav-product-tab',
			architectural: '#nav-product-tab',
			'architectural-design': '#nav-product-tab',
			uiux: '#nav-uiux-tab',
			'ui-ux': '#nav-uiux-tab',
			design: '#nav-uiux-tab',
			graphic: '#nav-uiux-tab',
			'graphic-design': '#nav-uiux-tab',
		};
		const filterTab = document.querySelector(portfolioFilterTabs[portfolioFilter.toLowerCase()]);

		if (filterTab) {
			if (window.bootstrap?.Tab) {
				new bootstrap.Tab(filterTab).show();
			} else {
				filterTab.click();
			}
		}
	...
	const scrollToHomeSection = (hash, updateHistory = true) => {
		...
		if (smoother) {
			smoother.scrollTo(hash, true, 'top 110px');
			return;
		}
	...
	document.addEventListener('click', (event) => {
		const link = event.target.closest('a[href="index.html#about"], a[href="index.html#services"]');
		if (!link) return;

		const url = new URL(link.href);
		const isHomePage = window.location.pathname.endsWith('/index.html') || window.location.pathname.endsWith('/');

		if (!isHomePage || url.pathname !== window.location.pathname) return;
	...
	if (window.location.hash === '#about' || window.location.hash === '#services') {
		const initialHomeSectionScroll = function () {
			setTimeout(function () {
				scrollToHomeSection(window.location.hash, false);
			}, 1700);
		};

		if (document.readyState === 'complete') {
			initialHomeSectionScroll();
		} else {
			windowOn.on('load', initialHomeSectionScroll);
		}
	}
  ...
  // 24. backToTop
	...
	if ($box.length) {
		...
		function updateBackToTop() {
			...
			let safeDocumentHeight = Math.max(documentHeight, 1);
			let percent = Math.min(
				Math.max(Math.floor((scrollPosition / safeDocumentHeight) * 100), 0),
				100
			);
			...
			if (scrollPosition >= 50) {
				$box.addClass("active-progress").show();
			} else {
				$box.removeClass("active-progress").hide();
			}
		}

		$(window).on('scroll resize pageshow load', updateBackToTop);
		updateBackToTop();
